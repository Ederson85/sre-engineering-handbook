# Linux Concepts

Conceptual foundation for Linux troubleshooting and production operations from an SRE perspective.

---

## Contents

<!-- AUTO-INDEX:START -->

- [01 - Linux for SRE](01-linux-for-sre.md)
- [02 - Filesystem](02-filesystem.md)
- [03 - Processes](03-processes.md)
- [04 - Networking](04-networking.md)
- [05 - Logs](05-logs.md)
- [06 - Systemd](06-systemd.md)
- [07 - Troubleshooting](07-troubleshooting.md)
- [Filesystem Full](filesystem-full.md)
- [High CPU](high-cpu.md)
- [Memory Pressure](memory-pressure.md)
- [Server Health Check](server-health.md)

<!-- AUTO-INDEX:END -->

---

## How to Use

Start with the numbered concepts, then use the server health check material to connect the concepts to incident investigation.
